...
def update_dropdown():
    dropdown["values"] = [item["ID"] for item in load_items()]
...
def refresh_logs():
    ...
    with open(LOGS_FILE, newline="") as f:
        ...
        rows = list(reader)
        for row in reversed(rows):  # ✅ reverse for recent first
...
tk.Label(remove_frame, text="Item ID:", font=label_font).grid(row=0, column=0, sticky="w")
...
tk.Label(remove_frame, text="Item Name:", font=label_font).grid(row=0, column=2, sticky="w")
...
remove_qty_entry.grid(row=1, column=1, padx=(5, 15), pady=(10, 0))

tk.Label(remove_frame, text="Recipient:", font=label_font).grid(row=1, column=2, sticky="w", pady=(10, 0))
recipient_entry = tk.Entry(remove_frame, width=25)
recipient_entry.grid(row=1, column=3, padx=(5, 15), pady=(10, 0))

tk.Button(remove_frame, text="Remove Item", command=remove_item, width=15).grid(row=1, column=4, pady=(10, 0))
